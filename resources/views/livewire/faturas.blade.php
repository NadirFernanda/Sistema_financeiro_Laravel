                ...
                <form wire:submit.prevent="save" enctype="multipart/form-data">
                    <div class="row">
                        <div class="col-12 mb-3">
                            ...
                            @error('numero_factura') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-12 mb-3">
                            ...
                            @error('empresa_nome') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-12 mb-3">
                            ...
                            @error('tipo_servico') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-12 mb-3">
                            ...
                            @error('natureza') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-12 mb-3">
                            ...
                            @error('tipologia') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-12 mb-3">
                            ...
                            @error('data_execucao') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-12 mb-3">
                            ...
                            @error('data_pagamento') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-12 mb-3">
                            ...
                            @error('valor_total') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-12 mb-3">
                            ...
                            @error('valor_pago') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-12 mb-3">
                            ...
                            @error('observacoes') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-12 mb-3">
                            ...
                            <div style="position:relative;">
                                ...
                                <span id="arquivo-nome" style="margin-left:12px;vertical-align:middle;">
                                    {{ $arquivo ? (is_string($arquivo) ? $arquivo : (method_exists($arquivo, 'getClientOriginalName') ? $arquivo->getClientOriginalName() : 'Arquivo selecionado')) : 'Nenhum arquivo selecionado' }}
                                </span>
                            </div>
                            @error('arquivo') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>
        ...
        <table class="table mb-0" style="min-width:1200px;border-radius:16px;overflow:hidden;box-shadow:0 2px 8px rgba(24,119,242,0.04);">
            <thead style="background:#1877F2;color:#fff;font-weight:600;font-size:0.85rem;position:sticky;top:0;z-index:2;">
                <tr>
                    ...
                    @if($this->podeEditar || $this->podeInserir)
            ...
            <tbody>
                @forelse($faturas as $fatura)
                    <tr style="background:@if($loop->index % 2 == 0)#f4f8ff;@else #fff;@endif">
                        <td style="padding:14px 10px;vertical-align:middle;font-size:0.85rem;">{{ $fatura->numero_factura }}</td>
                        <td style="padding:14px 10px;vertical-align:middle;font-size:0.85rem;">{{ $fatura->empresa_nome }}</td>
                        <td style="padding:14px 10px;vertical-align:middle;font-size:0.85rem;">{{ $fatura->tipo_servico }}</td>
                        <td style="padding:14px 10px;vertical-align:middle;font-size:0.85rem;">{{ $fatura->natureza }}</td>
                        <td style="padding:14px 10px;vertical-align:middle;font-size:0.85rem;">{{ $fatura->tipologia }}</td>
                        <td style="padding:14px 10px;vertical-align:middle;font-size:0.85rem;">{{ $fatura->data_execucao }}</td>
                        <td style="padding:14px 10px;vertical-align:middle;font-size:0.85rem;">{{ $fatura->data_pagamento }}</td>
                        <td style="padding:14px 10px;vertical-align:middle;font-size:0.85rem;">{{ $fatura->valor_total }}</td>
                        <td style="padding:14px 10px;vertical-align:middle;font-size:0.85rem;">{{ $fatura->valor_pago }}</td>
                        <td style="padding:14px 10px;vertical-align:middle;font-size:0.85rem;">{{ $fatura->valor_pendente }}</td>
                        <td style="padding:14px 10px;vertical-align:middle;font-size:0.85rem;">{{ $fatura->observacoes }}</td>
                        <td style="padding:14px 10px;vertical-align:middle;font-size:0.85rem;">{{ $fatura->status }}</td>
                        <td style="padding:14px 10px;vertical-align:middle;font-size:0.85rem;">
                            @if($fatura->arquivo)
                                <a href="{{ asset('storage/' . $fatura->arquivo) }}" target="_blank" style="color:#1877F2;font-weight:500;text-decoration:underline;">Ver arquivo</a>
                            @else
                        ...
                        @if($this->podeEditar || $this->podeInserir)
                            <td style="padding:14px 10px;vertical-align:middle;">
                                <div style="display:flex;flex-direction:column;align-items:flex-start;gap:8px;">
                                    @if($this->podeEditar)
                                        <button class="btn btn-sm" style="background:#1877F2;color:#fff;border:none;border-radius:8px;padding:7px 18px;font-weight:600;transition:background 0.2s;cursor:pointer;z-index:10;position:relative;" wire:click="edit('{{ $fatura->numero_factura }}')">Editar</button>
                                    @endif
                                    @if(auth()->user() && in_array(strtolower(auth()->user()->role), ['admin', 'secretaria']))
                                        <button class="btn btn-sm" style="background:#e74c3c;color:#fff;border:none;border-radius:8px;padding:7px 18px;font-weight:600;transition:background 0.2s;cursor:pointer;z-index:10;position:relative;"
                                            ...
                                            wire:click="delete('{{ $fatura->numero_factura }}')">
                                            Excluir
                                        </button>
                                    @endif
                                </div>
